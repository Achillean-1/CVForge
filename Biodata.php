...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($formUsername) || empty($formEmail) || empty($formNotelp) || empty($formImageName) || empty($formAlamat)) {
        ...
        if ($emailExists) {
            ...
            // Email belum ada dalam database, lakukan penambahan (insert) data baru
            $stmt = $conn->prepare("INSERT INTO settings (username, email, notelp, image_pp, alamat) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $formUsername, $formEmail, $formNotelp, $formImageName, $formAlamat);
        }
